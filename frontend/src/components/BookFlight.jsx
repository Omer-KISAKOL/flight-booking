...
import {convertToAmPm, extractTimeFromDateTime, FlightTimeDifference} from "../utils/timeUtils.jsx";
...
export const BookFlight = ({ flight, onClose , handleFlightSubmit , transferAirport, finalDestination}) => {
    ...
    return (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white p-8 rounded-lg w-[65%] shadow-lg relative">
                <button className="bg-transparent my-2 mx-4 rounded hover:text-red-800 absolute top-3 right-4" onClick={onClose}>
                ...
                <div className="mt-4">
                    ...
                    <select className="border border-gray-300 p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-600 w-52">
                ...
                <div className="mt-6 flex relative">
                    ...
                    <div className="text-xl font-bold text-blue-600 absolute bottom-0 left-1/2 transition-opacity -ml-20 px-6 py-4 shadow-xl border-2 border-blue-600 rounded-2xl">
                        {filters.message && <p>{filters.message}</p>}
                    </div>
                </div>
                <div>
                    <Link className="px-4 py-2 rounded-book border-2 border-purple-800 hover:bg-purple-800 hover:text-white" to="/Flights">My Flights</Link>
                </div>
            </div>
        </div>
    );
